...
for c in configurations:
    ...
    # for pno in np.arange(0,30):
        ...
        file_name = f"results/convergence-CLRL-3-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-0-{c['loadFileName']}-{c['reward_func']}-{c['credit_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
        data_RL.append(data)
        ...
        file_name = f"results/convergence-CLRL-3-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-1-{c['loadFileName']}-{c['reward_func']}-{c['credit_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
