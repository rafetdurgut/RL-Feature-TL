...
parameters = {"pNo":np.arange(500,5001,250),"Method": ["extreme"], "W": [250], "eps": [0.3], "alpha": [0.5],"gama": [0.5],"learning_mode":[0],"loadFileName":["CLRL-4-extreme-0.3-250-0.5-0.5-1-None-1-2500.csv"],"reward_func":[1]}
...
for c in configurations:
    ...
    # for pno in np.arange(0,30):
        ...
        file_name = f"results/convergence-CLRL-4-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-0-{c['loadFileName']}-{c['reward_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
        data_RL.append(data)
        ...
        file_name = f"results/convergence-CLRL-4-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-1-{c['loadFileName']}-{c['reward_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
    ...
    print(data_CRL_max)
    print('-'*50)
    print(sum(map(lambda x : x<0.05, ss)))
