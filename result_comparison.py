...
parameters = {"pNo":[0],"Method": ["extreme"], "W": [25], "eps": [0.3], "alpha": [0.9],"gama": [0.5],"learning_mode":[0],"loadFileName":["CLRL-4-extreme-0.3-25-0.9-0.5-1-None-1-1-2500.csv"],"reward_func":[0,1],"credit_func":[0,1],"load_func":[0]}
...
for c in configurations:
    ...
    # for pno in np.arange(0,30):
        ...
        file_name = f"results/convergence-CLRL-3-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-0-{c['loadFileName']}-{c['reward_func']}-{c['credit_func']}-{c['load_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
        data_RL.append(data)
        ...
        file_name = f"results/convergence-CLRL-3-{c['Method']}-{c['eps']}-{c['W']}-{c['alpha']}-{c['gama']}-1-{c['loadFileName']}-{c['reward_func']}-{c['credit_func']}-{c['load_func']}-{problem.dosyaAdi}.csv"
        data =get_best_data(file_name, 3)
